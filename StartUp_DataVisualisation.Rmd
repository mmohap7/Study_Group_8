...
After the data cleaning we have assigned it back to the original data set and then start our exploratory data analysis. We will treat any outliers as we go through the EDA.
```{r}
...
### Funding Amount
First, we looked into the distribution of the funding amount. It is highly right-skewed, which is why we chose to also visualise this with a log function to check the distribution
```{r}
...
Looking at the funding total, there are some outliers. These do not seem unjustified, after looking these companies up, so we left them as they were.
...
We explore a few variables by category to see what  criteria we could utilize to predict which categories of startups are more likely to be successful. We look into average funding raised, number of funding rounds, number of milestones, number of startups in top 500 list, number of acquired v.s. failed startups, and total funding.

```{r category exploration}
...
# startup acquisition by category
 startup_acquisition <- startup_data_process %>% group_by(category_code)%>%summarise(total_acquired= sum(status=='acquired'), total_not_acquired = sum(status=='closed'))
...
# startup succes rate by category
 startup_success_rate <- startup_data_process %>% group_by(category_code)%>%summarise(total_acquired= sum(status=='acquired'), total_not_acquired = sum(status=='closed'))
...
After inspection on the above variables,we decide to focus on three main parameters that could measure the success of a startup: average funding raised, number of startups in top 500 list, number of acquired v.s. failed startups (success rate). By analysing the patterns in these three variables, we can look into what type of startups have higher chances to get funded, get acquired and develop into a large-scale company.

```{r category chart}
# Average amount of funding raised per category
# To compare the absolute amount of average funding, bar chart(sorted in descending order) is the most straightforward way for human eyes to see the order and comparison
...
theme(
    ...
    plot.subtitle = element_text(face = "bold", size = 10, colour = "#4aaec0"),
...
# no. of startups in the 'top 500' list
# To compare the absolute number of startups in the Top500 list, bar chart(sorted in descending order) is the most straightforward way for human eyes to see the order and comparison
...
percentData <- startup_data_process %>% 
...
startup_sucess_plot <- startup_data_process %>%
  filter(category_code %in% startup_acquisition$category_code)%>%
  ggplot(aes(fill = status, x = count(category_code), y = category_code)) + 
  geom_bar(position = "fill", stat = "identity") + 
  geom_text(data = percentData, aes(x = n, label = ratio), stat = "identity", position = position_fill(vjust = 0.5)) +
