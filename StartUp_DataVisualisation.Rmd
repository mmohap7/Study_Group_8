...
success_rate_plot <- success_rate %>% 
  ggplot(aes(
    ...
    x = reorder(category2,count/sum(count)*1000),
    fill = reorder(status, desc(status))
   )) +
...
success_rate_plot2 <- startup_success_rate %>% 
  ggplot(aes(
    y = success_ratio,
    x = category_code,
    fill = category_code)) +
  geom_bar(stat="identity",fill="#4aaec0",position = "dodge") + 
  coord_flip() +
  scale_fill_manual(values = c("#a5a8b6", "#649c23")) +
  labs(fill = "Status",
       x="",
  y="Success Rate",
  title = "Analytics startups have the highest chance to succeed",
       subtitle = "Startup Success Rate by Category") + 
  theme_minimal()+
  NULL
success_rate_plot2
